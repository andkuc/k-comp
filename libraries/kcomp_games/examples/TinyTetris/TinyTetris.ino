...
bool processKeys() {
  ...
  int dpadpos = KCompKeypad::getPos();

  //Serial.println(dpadpos);

  switch(dpadpos) {
    case KEY_LEFT:
      if( KCompKeypad::DoDebounce() ) {
        acceleration = KCompKeypad::setAccel(acceleration, leftRight);
      }
      movePieceLeft();
    break;
    case KEY_RIGHT:
    ...
    case KEY_DOWN:
      if( KCompKeypad::DoDebounce() ) {
        acceleration = KCompKeypad::setAccel(acceleration, down);
      }
      movePieceDown();
    break;
    case KEY_ROTATE:
      if( KCompKeypad::DoDebounce() ) {
        acceleration = KCompKeypad::setAccel(acceleration, rotate);
      }
      RotatePiece();
    break;
    default:
...
void loop() {
  ...
  while (!gameOver) {
    ...
    int delay = analogRead(POT);
    while(millis() - moveTime < delay) {
