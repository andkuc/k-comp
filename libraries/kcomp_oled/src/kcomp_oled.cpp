...
// ==============================================================================
// 🧠 FULL SHADOW BUFFER (1024 Bytes)
// ==============================================================================
static uint8_t _oled_buffer[1024];
...
void clearDisplay() {
    memset(_oled_buffer, 0, 1024);
}

void updateDisplay() {
    // 1. Reset hardware pointers to Top-Left
    Wire.beginTransmission(OLED_I2C_ADDRESS);
    Wire.write(0x00);
    ...
    // 2. Blast the 1024 bytes in robust 16-byte chunks
    uint8_t *ptr = _oled_buffer;
...
// OPTIMIZED: Stripped of state variables. Only updates the hardware flag.
...
void drawPixel(int16_t x, int16_t y, uint8_t color) {
    ...
    switch (color) {
        case OLED_COLOR_WHITE:  _oled_buffer[x + (y / 8) * OLED_WIDTH] |=  (1 << (y & 7)); break;
        case OLED_COLOR_BLACK:  _oled_buffer[x + (y / 8) * OLED_WIDTH] &= ~(1 << (y & 7)); break;
        case OLED_COLOR_INVERT: _oled_buffer[x + (y / 8) * OLED_WIDTH] ^=  (1 << (y & 7)); break;
    }
}

bool getPixel(int16_t x, int16_t y) {
    if ((x < 0) || (x >= CURRENT_WIDTH) || (y < 0) || (y >= CURRENT_HEIGHT)) return false;
    ...
    return (_oled_buffer[x + (y / 8) * OLED_WIDTH] & (1 << (y & 7)));
}
...
void drawFastVLine(int16_t x, int16_t y, int16_t h, uint8_t color) { drawLine(x, y, x, y + h - 1, color); }
void drawFastHLine(int16_t x, int16_t y, int16_t w, uint8_t color) { drawLine(x, y, x + w - 1, y, color); }
...
void printChar(char c, uint8_t color, uint8_t size) {
    if ((_cursor_x >= CURRENT_WIDTH) || (_cursor_y >= CURRENT_HEIGHT) || ((_cursor_x + 5 * size - 1) < 0) || ((_cursor_y + 8 * size - 1) < 0)) return;
